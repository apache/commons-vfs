...
public final class SftpTestServerHelper {
    ...
    /**
     * Custom {@link SftpFileSystemAccessor} that tracks POSIX permissions in memory so that {@code setExecutable}/{@code setReadable}/{@code setWritable}
     * round-trip correctly on platforms without native POSIX support (e.g.&nbsp;Windows).
     */
    private static class TestSftpFileSystemAccessor implements SftpFileSystemAccessor {

        private static final ConcurrentHashMap<String, Set<PosixFilePermission>> permissionsCache = new ConcurrentHashMap<>();

        static void clearCache() {
            permissionsCache.clear();
        }

        private static Set<PosixFilePermission> defaultPermissions(final Path file, final LinkOption... options) {
        ...
        @Override
        public Map<String, ?> readFileAttributes(final SftpSubsystemProxy subsystem, final Path file, final String view, final LinkOption... options)
                throws IOException {
            Map<String, ?> result;
            ...
            try {
                ...
                final Map<String, Object> attrs = new HashMap<>(Files.readAttributes(file, "basic:*", options));
            ...
            if (view.startsWith("unix:") || view.startsWith("posix:") || synthetic) {
                ...
                final Set<PosixFilePermission> cached = permissionsCache.get(file.toAbsolutePath().toString());
        ...
        @Override
        public void removeFile(final SftpSubsystemProxy subsystem, final Path path, final boolean isDirectory) throws IOException {
            permissionsCache.remove(path.toAbsolutePath().toString());
            SftpFileSystemAccessor.super.removeFile(subsystem, path, isDirectory);
        }

        @Override
        public NavigableMap<String, Object> resolveReportedFileAttributes(final SftpSubsystemProxy subsystem, final Path file, final int flags,
        ...
        @Override
        public void setFileAttribute(final SftpSubsystemProxy subsystem, final Path file, final String view, final String attribute, final Object value,
        ...
        @Override
        public void setFileOwner(final SftpSubsystemProxy subsystem, final Path file, final Principal owner, final LinkOption... options) throws IOException {
        }

        @Override
        public void setFilePermissions(final SftpSubsystemProxy subsystem, final Path file, final Set<PosixFilePermission> perms, final LinkOption... options)
                throws IOException {
            permissionsCache.put(file.toAbsolutePath().toString(), new HashSet<>(perms));
        }

        @Override
        public void setGroupOwner(final SftpSubsystemProxy subsystem, final Path file, final Principal group, final LinkOption... options) throws IOException {
    ...
    public static synchronized void startServer() throws IOException {
        ...
        server.setPort(0);
        final SimpleGeneratorHostKeyProvider keyProvider = new SimpleGeneratorHostKeyProvider(tmpDir.resolve("hostkey.ser"));
